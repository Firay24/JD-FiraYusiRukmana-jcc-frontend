...
const CreateEvent = () => {
  ...
  const handleKonfirmasi = async () => {
    const data = {
      studentId: "7c0046f3f8a94072be75",
  ...
  return (
    <div className="min-h-screen bg-base-gray p-4">
      <Container>
          ...
          <div className="rounded-xl border bg-white p-7">
            ...
            <div className="mt-8 flex flex-col items-center gap-2">
              ...
              {fields[0].subject === "" || fields[0].level === "" ? (
                <button disabled className="mt-3 w-full cursor-not-allowed rounded-lg bg-base-darkPurple p-2 text-white opacity-50">
                  Daftar
                </button>
              ) : (
                <button onClick={handleKonfirmasi} disabled={isLoading} className="mt-3 flex w-full items-center justify-center rounded-lg bg-base-darkPurple p-2 text-white">
                  {isLoading ? <span className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent"></span> : "Daftar"}
                </button>
              )}
            </div>
          </div>
                  ...
                  <tbody>
                    <tr className="bg-white dark:border-gray-700 dark:bg-gray-800">
                      <th scope="row" className="whitespace-nowrap px-6 py-4 font-medium text-gray-900 dark:text-white">
                        {fields[0].subject.charAt(0).toUpperCase() + fields[0].subject.slice(1)}
                      </th>
                      <td className="px-6 py-4">1</td>
                      <td className="px-6 py-4">Rp70.000</td>
                    </tr>
                  </tbody>
                </table>
              </div>
        ...
        {isDaftar && (
          <div className="mt-5 flex flex-col gap-2 rounded-lg bg-white p-4">
            ...
            <div>
              ...
              <p className="text-lg font-normal text-black">Kania Rahma</p>
            </div>
          </div>
        )}

        {/* Status Transaksi */}
        {isDaftar && statusInvoice && (
        ...
        {isDaftar && (
          <button className="bg-primary mt-5 w-full rounded-lg bg-base-darkBlue p-2 text-white" onClick={handleKonfirmasi}>
